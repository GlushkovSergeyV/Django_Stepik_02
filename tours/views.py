...
def main_view(request):
    tourslist = {tourkey: tours[tourkey] for tourkey in random.sample(range(1, len(tours) + 1), 6)}
    context = {"title": title,
               "subtitle": subtitle,
...
def departure_view(request, departure):
    toursfilter = {tourkey: tours[tourkey] for tourkey, tourval in tours.items() if tourval["departure"] == departure}
    tourmincost = min(toursfilter.values(), key=lambda x: x["price"])["price"]
    tourmaxcost = max(toursfilter.values(), key=lambda x: x["price"])["price"]
    tourminnights = min(toursfilter.values(), key=lambda x: x["nights"])["nights"]
    tourmaxnights = max(toursfilter.values(), key=lambda x: x["nights"])["nights"]

    context = {"title": title,
               "subtitle": subtitle,
               "description": description,
               "departures": departures,
               "dep": departures[departure][3::],
               "tours": toursfilter,
               "tourmincost": tourmincost,
               "tourmaxcost": tourmaxcost,
               "tourminnights": tourminnights,
               "tourmaxnights": tourmaxnights
               }
